...
void doLoginResponse(json &responsejs); // 处理登录的响应逻辑
void doRegResponse(json &responsejs);   // 处理注册的响应逻辑


// 聊天客户端程序实现，main线程用作发送线程，子线程用作接收线程
int main(int argc, char **argv){
    // 判断命令是否合法，需要接受两个参数 ip port
    if (argc < 3){
    ...
    server.sin_port = htons(port);  // 将主机字节序转化为网络字节序(network short)
    ...
    for (;;){
        ...
        switch (choice){
            case 1:     // login业务
            {
                ...
                cin.get();  // 读掉缓冲区残留的回车
                cout << "userpassword:";
                cin.getline(pwd, 50);   // 这里貌似会读取到回车字符
            ...
            case 2:     // register业务
            {
                ...
                char pwd[50] = {0};
                cout << "username:";
                cin.getline(name, 50);      // 之所以用getline(c语言里用gets)，以为cin、scanf遇到空格会结束输入
                cout << "userpassword:";
                cin.getline(pwd, 50);
...
void mainMenu(int clientfd)
{
    ...
    {
        cin.getline(buffer, 1024);  // 输入，getline可以捕捉空格字符
        ...
        it->second(clientfd, commandbuf.substr(idx + 1, commandbuf.size() - idx));  // 调用命令处理方法(这里貌似是函数指针？)
    }
}
...
void addfriend(int clientfd, string str)
{
    ...
    int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);    // +1 是因为strlen不包括结尾字符'\0'或者'\n'
    if (-1 == len)
    {
...
void loginout(int clientfd, string)
{
    ...
    if (-1 == len){
        ...
        isMainMenuRunning = false;  // 退出 主聊天页面程序，循环到首页面
    }   
}
